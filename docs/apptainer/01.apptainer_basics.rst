...
.. Note::
    ...
    Prerequisites
	This section uses the |machine| compute cluster to run Apptainer. An active allocation on |machine| is required, though most content will apply to any system that supports Apptainer.
...
The Apptainer module is currently only available on compute nodes at |site|. To use Apptainer interactively, start an interactive session on a compute node using the |interactivecmd| command.
