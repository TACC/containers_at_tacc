...
by listing the current builders.
...
Notice the warning message printed at the end of the log. Since you can't have multiple images of same name with
different architectures, it doesn't load anything into Docker (running ``docker images`` would confirm that) and
